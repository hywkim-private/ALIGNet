...
def L_TV_Loss_3d(diff_grid, grid_size, lambda_):
  #create the identity differential grid  
  batch, _,d,h,w = diff_grid.shape
  diff_i_grid = ops_3d.init_grid_3d(grid_size)
  diff_i_grid = diff_i_grid.view(3,grid_size,grid_size,grid_size)
  diff_i_grid_z = diff_i_grid[0]
  diff_i_grid_y = diff_i_grid[1]
  diff_i_grid_x = diff_i_grid[2]
  L_TV_Loss = torch.norm(diff_grid[:,0] - diff_i_grid_z, p=1) + torch.norm(diff_grid[:,1] - diff_i_grid_y, p=1) + torch.norm(diff_grid[:,2] - diff_i_grid_z, p=1)
  L_TV_Loss = L_TV_Loss / batch
  L_TV_Loss = L_TV_Loss / (d*w*h) 
  L_TV_Loss = L_TV_Loss * lambda_
  #print(f"L_TV_Loss_3d {L_TV_Loss}")
  return L_TV_Loss
  
def get_loss_3d(tar_img, tar_est, diff_grid, grid_size, vox_size):
  L2_Loss_ = L2_Loss_3d(tar_img, tar_est, vox_size)
  L_TV_Loss_ = L_TV_Loss_3d(diff_grid, grid_size, 1e-4)
